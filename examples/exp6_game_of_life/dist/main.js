...
quiv.setNodes({"GAME_OF_LIFE":{"key":"GAME_OF_LIFE","next":["CONTEXT"],"prev":null,"level":0,"group":0,"type":"root"},"CONTEXT":{"key":"CONTEXT","next":["fn[0]"],"prev":"GAME_OF_LIFE","level":1,"group":0,"type":"branch"},"fn[0]":{"key":"fn[0]","next":["GAME_LOOP"],"prev":"CONTEXT","level":2,"group":0,"type":"branch"},"GAME_LOOP":{"key":"GAME_LOOP","next":["fn[1]"],"prev":"fn[0]","level":3,"group":0,"type":"branch"},"fn[1]":{"key":"fn[1]","next":["fn[2]"],"prev":"GAME_LOOP","level":4,"group":0,"type":"branch"},"fn[2]":{"key":"fn[2]","next":[],"prev":"fn[1]","level":5,"group":0,"type":"leaf"},"RULES":{"key":"RULES","next":["DO_NOTHING","REVIVE","DIE"],"prev":null,"level":0,"group":1,"type":"root"},"DO_NOTHING":{"key":"DO_NOTHING","next":[],"prev":"RULES","level":1,"group":1,"type":"leaf"},"REVIVE":{"key":"REVIVE","next":[],"prev":"RULES","level":1,"group":1,"type":"leaf"},"DIE":{"key":"DIE","next":[],"prev":"RULES","level":1,"group":1,"type":"leaf"},"SET_IS_ALIVE":{"key":"SET_IS_ALIVE","next":[],"prev":null,"level":0,"group":2,"type":"root"},"CREATE_CELL":{"key":"CREATE_CELL","next":[],"prev":null,"level":0,"group":3,"type":"root"},"ITERATE_GRID":{"key":"ITERATE_GRID","next":[],"prev":null,"level":0,"group":4,"type":"root"},"IS_ALIVE":{"key":"IS_ALIVE","next":[],"prev":null,"level":0,"group":5,"type":"root"}});
...
if(![{ "rule": [1, 0] }].some((predicate) => quiv.test.isEqual(predicate, value, { partial: true }))) return undefined;
return (cells[current].nextAlive = cells[current].alive)
}
quiv.fn["REVIVE"] = (value, key, prev, next) => {
const { cells, current } = value;
if(![{ "rule": [0, 1] }].some((predicate) => quiv.test.isEqual(predicate, value, { partial: true }))) return undefined;
return (cells[current].nextAlive = true)
}
quiv.fn["DIE"] = (value, key, prev, next) => {
const { cells, current } = value;
if(![{ "rule": [0, 0] }].some((predicate) => quiv.test.isEqual(predicate, value, { partial: true }))) return undefined;
