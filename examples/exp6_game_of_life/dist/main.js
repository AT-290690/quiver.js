...
quiv.setNodes({
  ...
  GAME_LOOP: {
    key: 'GAME_LOOP',
    next: ['CHECK_SURROUNDINGS'],
    prev: 'CREATE_GRID',
    level: 2,
    type: 'branch'
  },
  CHECK_SURROUNDINGS: {
    key: 'CHECK_SURROUNDINGS',
    next: ['DRAW'],
    prev: 'GAME_LOOP',
    level: 3,
    type: 'branch'
  ...
  IS_ALIVE: { key: 'IS_ALIVE', next: [], prev: null, level: 0, type: 'root' }
});

quiv.arrows['GAME_OF_LIFE'] = async (value, key, prev, next) => {
...
quiv.arrows['CREATE_GRID'] = async (value, key, prev, next) => {
...
quiv.arrows['GAME_LOOP'] = async (value, key, prev, next) => {
  return value;
};
quiv.arrows['CHECK_SURROUNDINGS'] = async (value, key, prev, next) => {
  const { col, row, gameObjects } = value;

  for (let x = 0; x < col; x++) {
    for (let y = 0; y < row; y++) {
      // Count the nearby population
      const numAlive = await quiv.arrows['IS_ALIVE']({
        x,
        y,
        col,
        row,
...
quiv.arrows['DRAW'] = async (value, key, prev, next) => {
...
quiv.arrows['CREATE_CELL'] = async (value, key, prev, next) => {
  const { x, y } = value;
  // Store the position of this cell in the grid
  // Make random cells alive
  return { x, y, alive: Math.random() > 0.5 };
};
quiv.arrows['IS_ALIVE'] = async (value, key, prev, next) => {
  ...
  return [
    ...
    return (
      result +
      (X < 0 || X >= col || Y < 0 || Y >= row || !gameObjects[X + Y * col].alive
        ? 0
        : 1)
    );
  }, 0);
