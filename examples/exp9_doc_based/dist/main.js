...
quiv.setNodes({
  SETUP: { key: 'SETUP', next: [], prev: null, level: 0, type: 'root' },
  'AGE[PARAMS]': {
    key: 'AGE[PARAMS]',
    next: ['AGE[WORK]'],
    prev: null,
  ...
  INFO: { key: 'INFO', next: [], prev: 'REQUEST', level: 1, type: 'leaf' },
  AGE: { key: 'AGE', next: [], prev: 'REQUEST', level: 1, type: 'leaf' },
  SERVER: {
    key: 'SERVER',
    next: ['HANDLER'],
    prev: null,
...
quiv.arrows['INFO'] = async (value, key, prev, next) => {
  ...
  return !match.url(url, '/info') || !match.method(method, 'GET')
    ? void 0
    : await quiv.go('INFO[PARAMS]')(value);
};
quiv.arrows['AGE'] = async (value, key, prev, next) => {
  const { match, url, method } = value;
  return !match.url(url, '/age') || !match.method(method, 'POST')
    ? void 0
...
quiv.arrows['TEST'] = async (value, key, prev, next) => {
  ...
  const endUtilResult = mockRes.SETTINGS.end(mockRes.SETTINGS.res)
    .status(200)
    .send(1);
  suites['SETTINGS_END_0'] = isEqual(1, endUtilResult)
    ? success('Equal - End util returns the correct result', 1, endUtilResult)
    : fail('end utility returns the correct result', 1, endUtilResult);
