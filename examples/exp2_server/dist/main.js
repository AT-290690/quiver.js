...
__qvr.func['SERVER'] = async (value, key, prev, next) => {
  const init = {
    routes: {
      '/cat': 'CAT',
      '/age': 'AGE'
  ...
  const start = (req, res) => {
    ...
    req.on('end', () => {
      ...
      __qvr.restart();
      req.query = query;
      __qvr.go(__qvr.getRoot())({ method, req, res, init });
    });
  };
...
__qvr.func['AGE[POST](send)'] = async (value, key, prev, next) => {
...
__qvr.func['CAT'] = async (value, key, prev, next) => {
  return (value.match.url(value, '/cat') && value) || void 0;
};
__qvr.func['CAT[GET]'] = async (value, key, prev, next) => {
  return (value.match.method(value, 'GET') && value) || void 0;
};
__qvr.func['CAT[GET][all](validate)'] = async (value, key, prev, next) => {
  return (!('id' in value.query) && value) || void 0;
};
__qvr.func['CAT[GET][all](send)'] = async (value, key, prev, next) => {
  return value
    .end(value.res)
    .status(200)
    .send(value.toJSON(await readFile(value.DB_PATH, 'utf8')));
};
__qvr.func['CAT[GET][id](validate)'] = async (value, key, prev, next) => {
  return ('id' in value.query && value) || void 0;
};
__qvr.func['CAT[GET][id](send)'] = async (value, key, prev, next) => {
...
__qvr.func['CAT[POST]'] = async (value, key, prev, next) => {
  return (value.match.method(value, 'POST') && value) || void 0;
};
__qvr.func['CAT[POST](validate)'] = async (value, key, prev, next) => {
  if (!value.body)
    return void value
      .end(value.res)
      .status(403)
      .send({ message: 'No data provided!' });
  value.body = value.toJSON(value.body);
  if (!value.body.name || !value.body.age || !value.body.breed)
    return void value
      .end(value.res)
      .status(403)
...
__qvr.func['CAT[POST](send)'] = async (value, key, prev, next) => {
  const data = await readFile(value.DB_PATH, 'utf8');
  const json = value.toJSON(data);
...
__qvr.func['CAT[PUT]'] = async (value, key, prev, next) => {
  return (
    (value.match.method(value, 'PUT') && 'id' in value.query && value) || void 0
  );
};
__qvr.func['CAT[PUT](validate)'] = async (value, key, prev, next) => {
  if (!value.body)
    return void value
      .end(value.res)
      .status(403)
...
__qvr.func['CAT[PUT](send)'] = async (value, key, prev, next) => {
  const data = await readFile(value.DB_PATH, 'utf8');
...
__qvr.func['CAT[DELETE]'] = async (value, key, prev, next) => {
  return (value.match.method(value, 'DELETE') && value) || void 0;
};
__qvr.func['CAT[DELETE](validate)'] = async (value, key, prev, next) => {
  return (
    ('id' in value.query && value) ||
    void value.end(value.res).status(403).send({ message: 'No id provided!' })
  );
};
__qvr.func['CAT[DELETE](send)'] = async (value, key, prev, next) => {
  const data = await readFile(value.DB_PATH, 'utf8');
  const json = value.toJSON(data);
  const { id } = value.query;
  if (!json[id])
    return value.end(value.res).status(403).send({ message: 'Cat not found!' });
  delete json[id];
  await writeFile(value.DB_PATH, value.toString(json));
  value.end(value.res).status(200).send({ message: 'Cat deleted!' });
};

__qvr.func['REQUEST'] = async (value, key, prev, next) => {
  const { method, req, res, init } = value;
  const { match, end, routes, toJSON, tryCatch, toString, DB_DIR, DB_FILE } =
    init;
  ...
  __qvr.go(routes[url.split('?')[0]])(service);
};
export default () => {
  __qvr.setRoot(__qvr.nodes['SERVER'].key);
  __qvr.reset();
