...
qvr.setNodes({
  ...
  goTo: { key: 'goTo', next: [], prev: 'world', level: 3, type: 'leaf' },
  result: { key: 'result', next: ['out'], prev: null, level: 0, type: 'root' },
  out: { key: 'out', next: [], prev: 'result', level: 1, type: 'leaf' },
  log: { key: 'log', next: [], prev: null, level: 0, type: 'root' }
});
qvr.func['hello'] = async (__value, __key, __prev, __next) => {
  qvr.memo.collection = 3;
  return 'Hello';
};
...
qvr.func['goTo'] = async (__value, __key, __prev, __next) => {
  return (
    __value + ' ' + (await qvr.goTo('result', 2)) &&
    qvr.goTo('log', 3) &&
    qvr.visit('log')
  );
};
...
qvr.func['log'] = async (__value, __key, __prev, __next) => {
  return console.log(__value) || console.log(qvr.memo.collection);
};
